...
document_status: "implementation-candidate"
...
所有切片统一归 9.9.9，不再拆成 9.10。用户已在后续消息授权生成CC/CX 9.9.9候选发行包，供Claude fable5.1查看；安装态更新和推送不在本轮范围。
...
- AC1：CC-only / CX-only 都可独立闭环。检查：隔离另一端 CLI/配置后分别执行代表任务；无另一端账号或模型调用，无无关平台缺失门禁。
- AC2：多平台只增强且安装尊重选择。检查：单端选择不探测另一端认证；双端协作失联可回到可用本端，未完成结果不被接受。
- AC3：PACE 全消费者一致。检查：无 checklist 的 Feature 可按 design 完成；R/S 先 polish 后 review；biz 不调旧三件套；roadmap 不按模块数单独触发。
- AC4：state 有界、原子、可恢复。检查：≤12KiB/10条/160B；并发与溢出故障注入不丢原文；空更新零写；原指针能解析。
- AC5：同端恢复与跨端交接正确。检查：中断后找到实际基线、未提交改动与剩余动作；缺输入不宣称完成，不重建第二状态树。
- AC6：证据绑定实际输入。检查：改代码/合同/相关环境使原记录失效；缺来源/git失败为不可验证；有效业务证据可跨平台复用。
- AC7：一次独立 review 绑定最终内容。检查：本端 reviewer 可完成；缺失/错run/旧packet/输入变化不通过；接收者现场匹配持久派发记录；红区控制不被异步化。
- AC8：并行结果经过整合验证。检查：两个互斥 writer + 共享文件单写者，整合后测试；中断后从持久记录定位冲突归属、产物与整合者；真实id不凭名称猜。
- AC9：VM 传输与场景就绪可区分。检查：配置有但不通、通但服务未 ready、未提交输入不同步、超时清理等案例得到正确结果。
- AC10：VM 执行可回放且不全局必需。检查：本机与 dev RHEL 实跑承诺场景并回传/清理；无VM项目可本地闭环；required OS缺失不能假通过。
- AC11：全栈垂直切片可用。检查：真实 FE/BE/DB/权限链；正常/拒绝/越权/失败可核对；表设计和DDL、需求-证据映射齐全。
- AC12：发布与迁移诚实兼容。检查：单端/双端首装与9.9.8迁移回滚；用户覆盖保留；旧both可读；无受管重复入口；RELEASE/模板状态一致。
- AC13：行为质量与效率有基线。检查：按 eval-plan.md 预注册任务/条件/来源；质量不回退；非必要往返不增加；有重复工作的基线按预定规则减少，无自建遥测。
- AC14：承重平台能力逐端实测。检查：保存平台/版本/入口和关键hook、review、worktree、VM结果；文档或探测未知不冒充支持；不依赖Grok才完成。
