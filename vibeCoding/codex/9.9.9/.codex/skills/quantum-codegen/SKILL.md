...
- **有界修复**: 读失败 → 修 → 复验受影响项；同因连续三次失败保留 stderr/已试路径，回到 PACE 处理。
- **真实切片**: 按 [PACE 执行合同](../pace/references/execution-contracts.md) 核对准入/写集/整合；pack 检查或 mock demo 不等于真实 FE/BE/DB 通过。
